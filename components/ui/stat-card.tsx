...
const colorMap = {
  navy: {
    ...
    glow: 'from-[#0C1E3C]/20',
    ring: 'group-hover:ring-[#0C1E3C]/30',
  },
  cyan: {
    bg: 'bg-cyan-500',
    icon: 'text-white',
...
export function StatCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        'group relative overflow-hidden bg-card rounded-2xl p-5 border border-border',
        'card-shadow cursor-default',
    ...
    >
      ...
      <div className="relative flex items-start justify-between mb-4">
        ...
        >
          <Icon className={cn('w-4.5 h-4.5 transition-transform duration-300', colors.icon)} />
        </div>
      </div>

      <div className="relative flex items-end justify-between gap-2">
        <div>
          <p className="text-2xl font-bold text-foreground tracking-tight transition-transform duration-300 group-hover:translate-x-0.5">
  ...
  <p className="relative mt-1 text-xs text-muted-foreground">
    {changePeriod.charAt(0).toUpperCase() + changePeriod.slice(1)}
  </p>
)}
    </motion.div>
  )
