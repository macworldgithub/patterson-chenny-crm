...
//   serverUrl?: string;
//   /**
//    * Full simulation customer object (name, vehicle, brand, suburb,
//    * scenario, lastService, upgradeScore, id, phone, ...). This is what the
//    * backend's `data?.customer` expects on `start-session` in order to build
//    * the per-customer system prompt and prime the greeting.
//    */
//   customer?: Record<string, any> | null;
//   /**
//    * @deprecated Kept only so existing call sites still type-check. A bare
//    * vehicle string is NOT enough for the backend to build a customer-
//    * specific prompt — it reads `data.customer`, not `data.carContext`, so
//    * passing only this silently falls back to the generic "ask for name and
//    * vehicle" prompt for every call. Pass `customer` instead.
//    */
//   carContext?: string;
// }

// export function useVoiceAgent(options: UseVoiceAgentOptions = {}) {
//   const { serverUrl = "http://localhost:4030", customer = null } = options;
...
//   // Bumped on every clear so any in-flight playback loop knows to abandon itself
//   const playbackGenerationRef = useRef(0);
//   // FIX: holds the `resolve` for whichever chunk-playback Promise is
//   // currently being awaited inside playAudioQueue's while loop, so
//   // clearAudioPlayback can unblock it directly instead of relying on the
//   // (now-nulled) onended handler to fire resolve() for it.
//   const pendingResolveRef = useRef<(() => void) | null>(null);

//   // Keep the latest customer object in a ref so startSession() (a stable
//   // useCallback) always reads the current selection without needing to be
//   // recreated every time the user picks a different customer.
//   const customerRef = useRef<Record<string, any> | null>(customer);
//   useEffect(() => {
...
//       await new Promise<void>((resolve) => {
//         // FIX: stash resolve() so clearAudioPlayback() can call it directly
//         // on barge-in. Previously only `source.onended` called resolve(),
//         // but clearAudioPlayback nulls out onended before calling stop() —
//         // meaning stop() would never actually fire this resolve(), leaving
//         // this await (and the whole while-loop iteration) hung forever.
...
//       // If a barge-in landed while we were awaiting playback above, stop
//       // pulling further chunks for this (now-stale) generation.
//       if (myGeneration !== playbackGenerationRef.current) return;
//     }
...
//     // FIX: unblock any playAudioQueue() loop that's currently stuck awaiting
//     // the chunk we just force-stopped above. Without this, that loop's
//     // `await new Promise(...)` would never resolve (onended was just nulled
//     // out, so stop() won't call it), and it would sit there indefinitely
//     // instead of exiting on the generation check.
...
//   // ─── Start voice session ────────────────────────────────────────────────────
//   // FIX: previously emitted `{ carContext }` — just the vehicle string. The
//   // backend only reads `data?.customer` (the full customer object) to build
//   // the per-customer system prompt and prime the greeting with who's being
//   // called and why. Sending carContext meant the backend NEVER saw a
//   // customer object, so every call silently fell back to the generic
//   // "no customer supplied, ask for name and vehicle" prompt regardless of
//   // who was selected in the UI. Now sends the real customer object, and
//   // still lets a caller pass an explicit override if they want to.
//   const startSession = useCallback(
//     (customerOverride?: Record<string, any> | null) => {
//       if (!socketRef.current) return;
//       const customerPayload =
//         customerOverride !== undefined ? customerOverride : customerRef.current;
//       socketRef.current.emit("start-session", { customer: customerPayload || null });
//     },
//     [],
//   );
...
export function useVoiceAgent(options: UseVoiceAgentOptions = {}) {
  ...
  useEffect(() => {
    ...
    // FIX (root cause of the "transcript cuts off but audio keeps
    // speaking" bug): the server now emits this exactly once per NEW
    // agent turn (on OpenAI's `response.created`). This — not
    // "transcript-done" — is the correct moment to clear the running
    // transcript display, because it's the only point we can be certain
    // the PREVIOUS turn's text is finished and a genuinely new one is
    // starting. Clearing on "done" instead (the old behaviour) could wipe
    // out text that hadn't been merged into the message list yet if a
    // final event arrived slightly out of order.
    socket.on("agent-turn-start", () => {
      setState((prev) => ({ ...prev, agentTranscript: "" }));
    });
    ...
    // FIX: previously this handler reset agentTranscript to "" here. That
    // was the direct cause of truncated transcripts — if the reset ever
    // raced ahead of (or replaced) the last few accumulated deltas, the
    // UI was left showing only a partial sentence while ElevenLabs (fed
    // from the same deltas, but earlier in the pipeline) went on to
    // synthesize and speak the complete one. Now we finalize the display
    // to the server's authoritative full text (built from its own
    // accumulated buffer — the exact same text that was sent to
    // ElevenLabs) instead of blanking it. The actual reset for the next
    // turn happens on "agent-turn-start" above.
    socket.on("transcript-done", (data: any) => {
      console.log("[Voice] Agent transcript complete:", data.transcript);
      setState((prev) => ({
        ...prev,
        agentTranscript: data.transcript || prev.agentTranscript,
      }));
    });
