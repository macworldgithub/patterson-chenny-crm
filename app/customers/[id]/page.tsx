...
import {
  ChevronLeft, Phone, Mail, MapPin, Car, Star, Clock,
  CalendarCheck, FileText, MessageSquare, User, Loader2,
} from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import { StatusPill } from '@/components/ui/status-pill'
...
import { fetchCustomerById } from '@/lib/customers-api'
...
export default function CustomerProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      setError(null)
      try {
        const [customerData, callsData] = await Promise.all([
          fetchCustomerById(id),
          apiFetch(`/calls?customerId=${id}&limit=50`).then((r) =>
            r.data.map((c: any) => ({ ...c, id: c._id ?? c.id }))
          ),
        ])
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1200px] mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-2xl border border-border card-shadow overflow-hidden">
        <div className="bg-gradient-to-r from-[#0C1E3C] to-[#1A3A6B] p-6">
          <div className="flex items-start gap-4">
            <div className="w-16 h-16 rounded-2xl bg-cyan-500 flex items-center justify-center text-white text-2xl font-bold shrink-0">
              {customer.firstName[0]}{customer.lastName[0]}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 flex-wrap">
                <h1 className="text-2xl font-bold text-white">{customer.fullName}</h1>
                <StatusPill status={customer.status} />
                {customer.doNotCall && <span className="px-2 py-0.5 rounded-full bg-red-500/20 text-red-300 text-xs font-medium">DNC</span>}
              </div>
              <div className="flex items-center gap-4 mt-2 flex-wrap">
                <span className="text-white/60 text-sm flex items-center gap-1.5"><Mail className="w-3.5 h-3.5" />{customer.email}</span>
                <span className="text-white/60 text-sm flex items-center gap-1.5"><Phone className="w-3.5 h-3.5" />{customer.mobilePhone}</span>
                <span className="text-white/60 text-sm flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" />{customer.suburb}, {customer.state}</span>
              </div>
              <div className="flex items-center gap-3 mt-3">
                <UpgradeStars score={customer.upgradeScore} />
                <span className="text-white/60 text-xs">Upgrade Score {customer.upgradeScore}/5</span>
            ...
            <div className="hidden lg:flex flex-col gap-2 text-right shrink-0">
              <div className="bg-white/10 rounded-xl p-3">
                <p className="text-white/60 text-xs">Lifetime Value</p>
                <p className="text-white text-xl font-bold">${((customer.lifetimeValue || 0) / 1000).toFixed(0)}K</p>
              </div>
            </div>
          </div>
        </div>
        <div className="grid grid-cols-4 divide-x divide-border border-t border-border">
          {[
            { label: 'Total Spend', value: `$${((customer.totalSpend || 0) / 1000).toFixed(1)}K` },
            { label: 'Last Contact', value: customer.lastContactDate },
            { label: 'Dealership', value: (customer.assignedDealership || '').replace('Patterson Cheney ', '') },
            { label: 'Preferred Time', value: customer.preferredContactTime },
          ].map((s) => (
          ...
          <div className="bg-card rounded-2xl border border-border card-shadow p-5">
            ...
            {customer.vehicle ? (
              <div className="space-y-2.5">
                <div className="flex items-center gap-3 p-3 bg-muted/40 rounded-xl">
                  <div>
                    <p className="font-semibold text-sm text-foreground">{customer.vehicle.year} {customer.vehicle.make} {customer.vehicle.model}</p>
                    <p className="text-xs text-muted-foreground">{customer.vehicle.variant}</p>
                  </div>
                </div>
                {[
                  { label: 'Colour', value: customer.vehicle.color },
                  { label: 'Reg Plate', value: customer.vehicle.regPlate },
                  { label: 'Odometer', value: `${((customer.vehicle.odometer || 0) / 1000).toFixed(1)}k km` },
                  { label: 'Purchase Date', value: customer.vehicle.purchaseDate },
                  { label: 'Last Service', value: customer.vehicle.lastServiceDate },
                  { label: 'Next Service', value: customer.vehicle.nextServiceDue },
                  { label: 'Finance End', value: customer.vehicle.financeEndDate ?? 'N/A' },
              ...
              <p className="text-xs text-muted-foreground">No vehicle on record.</p>
            )}
          </div>
          <div className="bg-card rounded-2xl border border-border card-shadow p-5">
            <h3 className="font-semibold text-sm text-foreground mb-3">Tags</h3>
            <div className="flex flex-wrap gap-2">
              ...
              {(!customer.tags || customer.tags.length === 0) && <p className="text-xs text-muted-foreground">No tags.</p>}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          <Tabs defaultValue="calls" className="space-y-4">
            ...
            <TabsContent value="timeline">
              <div className="bg-card rounded-2xl border border-border p-5 space-y-4">
                {[
                  { date: customer.lastContactDate, icon: Phone, text: 'Outbound call — ' + (calls[0]?.outcome ?? 'N/A'), color: 'text-cyan-500' },
                  { date: customer.vehicle?.lastServiceDate, icon: CalendarCheck, text: 'Vehicle service completed', color: 'text-emerald-500' },
                  { date: customer.vehicle?.purchaseDate, icon: Car, text: `Purchased ${customer.vehicle?.year} ${customer.vehicle?.make} ${customer.vehicle?.model}`, color: 'text-blue-500' },
                  { date: (customer.createdAt || '').split('T')[0], icon: User, text: 'Customer record created', color: 'text-muted-foreground' },
                  ...
                  <div key={i} className="flex items-start gap-3">
                    ...
                    <div className="flex-1 pt-1">
                      <p className="text-sm text-foreground">{event.text}</p>
                      <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1"><Clock className="w-3 h-3" />{event.date}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-3">
                {calls[0]?.aiSummary && (
                  <div className="bg-card rounded-2xl border border-border p-5">
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">{calls[0].aiSummary}</p>
                  </div>
                )}
                <div className="bg-card rounded-2xl border border-border p-5">
                  <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed">{customer.notes || 'No notes recorded.'}</p>
                </div>
              </div>
            </TabsContent>
          </Tabs>
